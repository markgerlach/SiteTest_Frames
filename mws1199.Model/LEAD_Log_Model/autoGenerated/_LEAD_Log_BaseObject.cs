...
{
	[Serializable]
    public class LEAD_Log_BaseObject : IDisposable, 
		IClassGenCloneable, IClassGenPropertyChanged, INotifyPropertyChanged, IDXDataErrorInfo
    {
		...
		public bool IsValid
		{
			get 
			{ 
				if (this._disposed) { throw new ObjectDisposedException("LEAD_Log_BaseObject", _objectDisposedMessage); }
				...
				if (value != RecordStatus.Current)
				{
					...
					if (!RecordStatusParentHolders.TryGetValue(this.GetType(), out props))
					{
						...
						foreach (PropertyInfo prop in this.GetType().GetProperties())
						{
							if (prop.Name.EndsWith("Item") &&
								prop.PropertyType.FullName.Contains("LEAD_Log.") &&
					...
					foreach (PropertyInfo prop in props)
					{
						if (prop.GetValue(this, null) != null &&
							prop is LEAD_Log_BaseObject &&
							(LEAD_Log_BaseObject)prop.GetValue(this, null) != null)
						{
							switch (((LEAD_Log_BaseObject)prop.GetValue(this, null)).RecordStatus)
							{
								case RecordStatus.Current:
									((LEAD_Log_BaseObject)prop.GetValue(this, null)).RecordStatus = RecordStatus.Modified;
									break;
								default:
									((LEAD_Log_BaseObject)prop.GetValue(this, null)).RecordStatus =
										((LEAD_Log_BaseObject)prop.GetValue(this, null)).RecordStatus;
									break;
							}
						}
		...
		//{
		//    if (!LEAD_Log_BaseObject.PropInfoArray.ContainsKey(typeName))
		//    {
		//        // Load them up
		//        Type t = System.Type.GetType("LEAD_Log." + typeName);
		...
		//            }
		//        //}
		//        //((LEAD_Log_BaseObject)o).Dispose();		// Dispose of the object
		//        LEAD_Log_BaseObject.PropInfoArray.Add(typeName, newProps);		// Add the new collection
		//    }
		//}
...
#region ClassGenBindingList 
	...
	public interface IClassGenBindingList<T, K> where T : LEAD_Log_BaseObject
	{
		// Methods
		...
		where T : LEAD_Log_BaseObject
	{
		...
		public bool IsCurrent
		{
			get 
			{
				if (!(typeof(LEAD_Log_BaseObject).IsAssignableFrom(typeof(T))))
				{
					return true;
				}
				foreach (T item in this)
				{
					if ((item as LEAD_Log_BaseObject).RecordStatus != RecordStatus.Current)
					{
						return false;
					}
				}
		...
		public List<string> PropertyValueExistsMoreThanOnceInCollection(string propertyName, bool sortDescending)
		{
			...
			foreach (T item in this)
			{
				if ((item as LEAD_Log_BaseObject).RecordStatus != RecordStatus.Deleted)
				{
