...
namespace LEAD
{
		...
		IClassGenLogXML, IClassGenCopyWithNewID, IClassGenPropertyChanging, INotifyPropertyChanging
	{
		...
		private Agency _originalValues = null;

		private bool _disposed = false;

		// Child Items
		private ContactCollection _contactCollection = new ContactCollection();
		...
		/// <summary>
		/// Builds a Agencybased on a certain record from the database.
		/// </summary>
		/// <param name="agencyID">A primary key used to select the record.</param>
		public Agency(long agencyID)
		{
			this.Get(agencyID);
			...
			if (!this._disposed)
			{
				try
				{
					...
					if (disposing)
					{
						...
						if (_contactCollection != null && _contactCollection.IsDisposable) { _contactCollection.Dispose(); _contactCollection = null; }
					}
		...
		public ContactCollection Coll_Contact
		{
		...
		/// <summary>
		/// Property generated from the sAgencyKey field.
		/// </summary>
		public string AgencyKey
		{
			...
			set 
			{
				if (_agencyKey != value)
				{
					bool cancelled = NotifyPropertyChanging("AgencyKey", _agencyKey, value);
					if (!cancelled)
					{
						_agencyKey = value;
						SetModifiedStatus("AgencyKey");
						NotifyPropertyChanged("AgencyKey", value);
					}
				}
			}
		}
		...
		/// <summary>
		/// A Agency with all of this object's original values.
		/// </summary>
		[Newtonsoft.Json.JsonIgnore]
		public Agency OriginalValues
		{
		...
		public void SetBaseRules()
		{
			...
			this.Rules.Add(new PropertyRequiredCustomRule("AgencyID", "The \"Agency ID\" field is required in the Agency record.", RecordStatus.Modified));
			this.Rules.Add(new PropertyRequiredCustomRule("AgencyKey", "The \"Agency Key\" field is required in the Agency record."));
			this.Rules.Add(new PropertyRequiredCustomRule("Inactive", "The \"Inactive\" field is required in the Agency record."));
			this.Rules.Add(new PropertyRequiredCustomRule("DateAdded", "The \"Date Added\" field is required in the Agency record."));
			this.Rules.Add(new PropertyRequiredCustomRule("DateUpdated", "The \"Date Updated\" field is required in the Agency record."));
			this.Rules.Add(new PropertyRequiredCustomRule("Rowversion", "The \"Rowversion\" field is required in the Agency record.", RecordStatus.Modified));

			// Max Length Rules
			this.Rules.Add(new MaxLengthCustomRule("AgencyKey", 10, "The maximum length for the \"AgencyKey\" field is 10 characters."));
		...
		public ClassGenExceptionCollection BrokenRulesIncludingChildren
		{
			get 
			{
				...
				brokenRules.AddRange(_contactCollection.BrokenRules);
		...
		public Agency Clone()
		{
			Agency item = base.CloneObjectWithIL<Agency>(this);
			item.OriginalValues = this.OriginalValues;

			// Do the sub-collections
			#region Sub Collections
			item.Coll_Contact.Clear();		// Clear the collection
			foreach (Contact c in this.Coll_Contact)
			{
				item.Coll_Contact.Add(c);
			}
		...
		public object CopyWithNewID()
		{
			...
			// Do the sub-collections
			#region Sub Collections
			for (int i = item.Coll_Contact.Count - 1; i >= 0; i--)
			{
				item.Coll_Contact[i] = (Contact)item.Coll_Contact[i].CopyWithNewID();
				item.Coll_Contact[i].AgencyID = item.AgencyID;
			}
			#endregion Sub Collections
		...
		public ClassGenExceptionCollection RefreshObject(int threshHoldInMinutes)
		{
			...
			if (((TimeSpan)currentTime.Subtract(_dateTimeObjectPopulated.Value)).TotalMinutes > threshHoldInMinutes)
			{
				...
				errors.AddRange(this.Get(_agencyID));
			}
		...
		public ClassGenExceptionCollection RefreshForce()
		{
			...
			if (!String.IsNullOrEmpty(this._agencyID.ToString()))
			{
				errors.AddRange(this.Get(_agencyID));	// Just do a get on the object
			}
			else
			{
		...
		public override void UndoChanges()
		{
			...
			_agencyKey = _originalValues.AgencyKey;
			...
			// Do the sub-collections
			#region Sub Collections
			this.Coll_Contact.Clear();		// Clear the collection
			foreach (Contact c in _originalValues.Coll_Contact)
			{
				this.Coll_Contact.Add(c);
			}
			#endregion Sub Collections
		}
		...
		public ClassGenExceptionCollection AddUpdate(SqlConnection oConn, SqlTransaction oTrans)
		{
			...
			{
				if (Coll_Contact.ModifiedRecords.Count > 0 || 
					Coll_Contact.NewRecords.Count > 0 || 
					Coll_Contact.DeletedRecords.Count > 0)
				{
			...
			if (_recordStatus == RecordStatus.Deleted)
			{
				try
				{
					...
					errors.AddRange(LEAD_BaseObject.DeleteImmediate("tAgency", "iAgencyID", _agencyID.ToString(), true));
				}
				catch (SqlException sqle) { errors.Add(new ClassGenException(sqle)); }
				catch (Exception ex) { errors.Add(new ClassGenException(ex)); }
			...
			if (_recordStatus == RecordStatus.New || _recordStatus == RecordStatus.Modified)
			{
				...
				if (errors.Count == 0 &&
					_recordStatus == RecordStatus.Modified)
				{
					byte[] timeStampFromTable = DAL.GetTimeStampFromTable("tAgency", "bRowversion",
						"iAgencyID = " + this.AgencyID + "");
				...
				try
				{
					...
					sql = "DECLARE @return_val BIGINT " + 
						"IF NOT EXISTS (SELECT iAgencyID FROM tAgency WHERE iAgencyID = @piAgencyID) " + 
						"BEGIN" + 
						"    INSERT INTO tAgency (sAgencyKey, bInactive, sAddedByUser, dtUpdated, sUpdatedByUser) " + 
						"    SELECT @psAgencyKey, @pbInactive, @psAddedByUser, @pdtUpdated, @psUpdatedByUser " + 
						"    SELECT @return_val = SCOPE_IDENTITY() " + 
						"END " + 
						"ELSE " + 
						"BEGIN " + 
						"    UPDATE tAgency SET " + 
						"        sAgencyKey = @psAgencyKey" + 
						...
						"     WHERE iAgencyID = @piAgencyID " + 
						"     SELECT @return_val = @piAgencyID " + 
						"END; SELECT @return_val AS ret_val, bRowversion FROM tAgency WHERE iAgencyID = @return_val";
					cmd = new SqlCommand(sql);

					cmd.Parameters.Add("@piAgencyID", SqlDbType.BigInt).Value = _agencyID;
					cmd.Parameters.Add("@psAgencyKey", SqlDbType.VarChar, 10).Value = _agencyKey;
					...
					foreach (DataRow row in dt.Rows)
					{
						...
						long oldRecID = _agencyID;

						_agencyID = (long)row["ret_val"];
						_rowversion = (byte[])(row["bRowversion"]);

						// Go through the sub collections and update the records
						foreach (Contact c in Coll_Contact) { if (c.AgencyID == oldRecID) { c.AgencyID = _agencyID; } }
					}
					dt.Dispose();		// Kill off the datatable
					
					// Add the update methods for all the sub collections
					#region Sub Collections
					errors.AddRange(Coll_Contact.AddUpdateAll(oConn, oTrans));
					#endregion Sub Collections
				}
				catch (SqlException sqle) { errors.Add(new ClassGenException(sqle)); }
				catch (Exception ex) { errors.Add(new ClassGenException(ex)); }
		...
		/// </summary>
		/// <param name="agencyID">A key field used to find the correct record.</param>
		/// <param name="includeParentPopulation">Whether or not to include values derived from a parent table.</param>
		public ClassGenExceptionCollection Get(long agencyID, bool includeParentPopulation)
		{
			ClassGenExceptionCollection errors = new ClassGenExceptionCollection();
			SqlCommand cmd = null;
			string sql = string.Empty;
			...
			try
			{
				sql = "SELECT [A].*, GETDATE() AS dateTimeObjectPopulated " + 
					"FROM tAgency [A] " + 
					"WHERE [A].[iAgencyID] = @piAgencyID " + 
					"";
				cmd = new SqlCommand(sql);
				cmd.Parameters.Add("@piAgencyID", SqlDbType.BigInt).Value = agencyID;
		...
		public ClassGenExceptionCollection Get(long agencyID)
		{
			return Get(agencyID, false);		// Get the item
		}
		...
		public ClassGenExceptionCollection Get(DataRow row, bool includeParentPopulation)
		{
			...
			try
			{
				...
				_agencyKey = (row["sAgencyKey"] != DBNull.Value ? row["sAgencyKey"].ToString() : string.Empty);
		...
		/// <summary>
		/// Get all children for the Contact class.
		/// </summary>
		public void GetChildren_Contact()
		{
			this.Coll_Contact.Clear();		// Contact
			this.Coll_Contact.GetFromDBByAgencyID(_agencyID);
			foreach (Contact item in this.Coll_Contact)
			{
				item.AgencyItem = this;
				item.AgencyItem.IsLoaded = true;
		...
		public ClassGenExceptionCollection DeleteAllChildCollections()
		{
			...
			// Delete the record
			errors.AddRange(LEAD_BaseObject.DeleteImmediate("tAgency", "iAgencyID", _agencyID.ToString(), false));
		...
		public DataRow ToDataRow(DataTable dt)
		{
			...
			return ToDataRowWithIL<Agency>(this, dt);
		}
		...
		/// The property's name as a static value.
		/// </summary>
		public static readonly string FN_AgencyKey = "AgencyKey";
		...
		public static string GetDBFieldName(AgencyField field)
		{
			...
			switch (field)
			{
				case AgencyField.AgencyID: fldName = "iAgencyID"; break;
				case AgencyField.AgencyKey: fldName = "sAgencyKey"; break;
				case AgencyField.Inactive: fldName = "bInactive"; break;
				case AgencyField.DateAdded: fldName = "dtAdded"; break;
				case AgencyField.AddedByUser: fldName = "sAddedByUser"; break;
				case AgencyField.DateUpdated: fldName = "dtUpdated"; break;
				case AgencyField.UpdatedByUser: fldName = "sUpdatedByUser"; break;
				case AgencyField.Rowversion: fldName = "bRowversion"; break;
			}
		...
		public static string GetDBFieldName(string field)
		{
			...
			// Add the elements
			if (field.ToLower() == "iagencyid") { fldName = AgencyField.AgencyID.ToString(); }
			else if (field.ToLower() == "sagencykey") { fldName = AgencyField.AgencyKey.ToString(); }
			else if (field.ToLower() == "binactive") { fldName = AgencyField.Inactive.ToString(); }
			else if (field.ToLower() == "dtadded") { fldName = AgencyField.DateAdded.ToString(); }
			else if (field.ToLower() == "saddedbyuser") { fldName = AgencyField.AddedByUser.ToString(); }
			else if (field.ToLower() == "dtupdated") { fldName = AgencyField.DateUpdated.ToString(); }
			else if (field.ToLower() == "supdatedbyuser") { fldName = AgencyField.UpdatedByUser.ToString(); }
			else if (field.ToLower() == "browversion") { fldName = AgencyField.Rowversion.ToString(); }
		...
		/// </summary>
		public static readonly string DB_TableName = "tAgency"; 
		...
		/// <param name="field">The field type to get.</param>
		/// <returns>A list of values from the collection (sorted by value).</returns>
		public static List<string> GetDistinctFromDB(AgencyField field)
		{
		...
		/// <returns>A list of values from the collection (sorted by value).</returns>
		public static List<string> GetDistinctFromDB(AgencyField field, string whereClause)
		{
		...
		public static DataTable GetDistinctFromDBAsDataTable(AgencyField[] fields, 
			string[] fieldAliases,
			AgencyField[] sortedFields, 
			string whereClause)
		{
			Dictionary<AgencyField, string> fieldNames = new Dictionary<AgencyField, string>();
			StringBuilder sbFields = new StringBuilder();
			StringBuilder sbWhereClause = new StringBuilder();
			int fieldCount = 0;
			foreach (AgencyField fld in fields)
			{
			...
			Dictionary<AgencyField, string> sortedFieldNames = new Dictionary<AgencyField, string>();
			StringBuilder sbSortedFields = new StringBuilder();
			foreach (AgencyField fld in sortedFields)
			{
		...
		public static DateTime? GetMinDateFromDB(AgencyMinMaxDateField field)
		{
		...
		/// <param name="whereClause">The where clause to fire against the database.</param>
		/// <returns>The single earliest DateTime from the collection.</returns>
		public static DateTime? GetMinDateFromDB(AgencyMinMaxDateField field, string whereClause)
		{
			AgencyField fld = (AgencyField)Enum.Parse(typeof(AgencyField),
				field.ToString(), true);
		...
		public static DateTime? GetMaxDateFromDB(AgencyMinMaxDateField field)
		{
		...
		/// <param name="whereClause">The where clause to fire against the database.</param>
		/// <returns>The single latest DateTime from the collection.</returns>
		public static DateTime? GetMaxDateFromDB(AgencyMinMaxDateField field, string whereClause)
		{
			AgencyField fld = (AgencyField)Enum.Parse(typeof(AgencyField),
				field.ToString(), true);
			string fldName = GetDBFieldName(fld);
		...
		public static decimal? GetMinNumberFromDB(AgencyMinMaxNumberField field)
		{
		...
		/// <param name="whereClause">The where clause to fire against the database.</param>
		/// <returns>The single least value from the collection.</returns>
		public static decimal? GetMinNumberFromDB(AgencyMinMaxNumberField field, string whereClause)
		{
			AgencyField fld = (AgencyField)Enum.Parse(typeof(AgencyField),
				field.ToString(), true);
			string fldName = GetDBFieldName(fld);
		...
		public static decimal? GetMaxNumberFromDB(AgencyMinMaxNumberField field)
		{
		...
		/// <param name="whereClause">The where clause to fire against the database.</param>
		/// <returns>The single greatest value from the collection.</returns>
		public static decimal? GetMaxNumberFromDB(AgencyMinMaxNumberField field, string whereClause)
		{
			AgencyField fld = (AgencyField)Enum.Parse(typeof(AgencyField),
				field.ToString(), true);
			string fldName = GetDBFieldName(fld);
		...
		/// <returns>A copy of the object, with a new ID.</returns>
		public static Agency CopyFrom(Agency source)
		{
			// Make the copy of the object
			Agency item = source.Clone();
		...
		public static string ToDBFriendly(string dbString)
		{
			...
			rtv = rtv.Replace("[AgencyKey]", "[sAgencyKey]");
		...
		/// <summary>
		/// Attempt to serialize the Agency object to the server
		/// </summary>
		/// <param name="obj">The Agency object to serialize</param>
		/// <returns>A string containing the return from the method</returns>
		public static string SerializeObject(Agency obj)
		{
		...
		/// <summary>
		/// Attempt to Deserialize the Agency object from the server
		/// </summary>
		/// <param name="obj">The Agency object to deserialize</param>
		/// <param name="errors">The errors collection</param>
		/// <returns>A Agency object containing the return from the method</returns>
		public static Agency DeserializeObject(string obj, ref ClassGenExceptionCollection errors)
		{
			Agency rtv = null;

			try
			{
				rtv = Newtonsoft.Json.JsonConvert.DeserializeObject<Agency>(obj);
			}
			catch (Exception ex)
			{
		...
		public string GetAsXML(bool includeBaseProperties)
		{
			...
			sb.Append("<AgencyKey>" + System.Security.SecurityElement.Escape(AgencyKey) + "</AgencyKey>" + Environment.NewLine);
			...
			{
				sb.Insert(0, "<Agency>" + Environment.NewLine);
				sb.Append("</Agency>" + Environment.NewLine);
			}
		...
		public string GetChangedElementsAsXML()
		{
			...
			if (_originalValues.AgencyKey != AgencyKey)
			{
				sb.Append("\t<AgencyKey>");
				sb.Append("<OldValue>" + _originalValues.AgencyKey + "</OldValue>");
				sb.Append("<NewValue>" + AgencyKey + "</NewValue>");
				sb.Append("</AgencyKey>" + Environment.NewLine);
			}
		...
		/// <summary>
		/// Generates a new Agency object from an XML description.
		/// </summary>
		/// <param name="xml">The string of XML to load from.</param>
		/// <returns>A new Agency object.</returns>
		public static Agency LoadFromXML(string xml)
		{
			...
			foreach (System.Xml.XmlNode child in doc.ChildNodes[0].ChildNodes)
			{
				...
				switch (child.Name)
				{
					...
					case "AgencyKey": item.AgencyKey = child.InnerText; break;
	...
	#endregion Agency Class

	#region AgencyField Enum
	/// <summary>
	/// Enum for each field type in the element.
	/// </summary>
	public enum AgencyField
	{
		/// <summary>
	...
	#endregion AgencyField Enum

	#region AgencyMinMaxDateField Enum
	/// <summary>
	/// Enum for each DateTime field in the element that can have a minimum or maximum.
	/// </summary>
	public enum AgencyMinMaxDateField
	{
		/// <summary>
		/// The DateAdded element.
		/// </summary>
	...
	#endregion AgencyMinMaxDateField Enum

	#region AgencyMinMaxNumberField Enum
	/// <summary>
	/// Enum for each numerical field in the element that can have a minimum or maximum.
	/// </summary>
	public enum AgencyMinMaxNumberField
	{
		/// <summary>
		/// The AgencyID element.
		/// </summary>
	...
	#endregion AgencyMinMaxNumberField Enum

	#region Agency Collection
	/// <summary>
	/// A collection for Agency objects.
	/// </summary>
	/// <remarks>
	/// This collection class is the primary way to work with Agency objects.
	/// Most of the functionality is in this class, including the methods for creating Agency objects from a database.
	/// </remarks>
	[Serializable]
	public partial class AgencyCollection : ClassGenBindingList<Agency, long>, IDisposable, 
		IClassGenLogXMLCollection, IClassGenClassGenerated, IClassGenClassUpdatable
	{
		...
		public AgencyCollection(string whereClause) 
		{
			// The Key Field Name for the collection
			this.KeyFieldName = "AgencyID";
		...
		public AgencyCollection(string whereClause, bool getChildren) 
		{
			// The Key Field Name for the collection
			this.KeyFieldName = "AgencyID";
		...
		/// </summary>
		/// <param name="agencyID">A key field used to find the correct record.</param>
		/// <returns>The Agency object that matches the given value.</returns>
		public Agency Get(long agencyID)
		{
			Agency val = null;
			if (KeyedCollection.ContainsKey(agencyID)) { val = KeyedCollection[agencyID]; }
			return val;
		}
		...
		/// </summary>
		public new AgencyCollection ActiveRecords
		{
			get
			{
				return (AgencyCollection)base.ActiveRecords;
			}
		}
		...
		/// </summary>
		public AgencyCollection DeletedRecords
		{
			get
			{
				return (AgencyCollection)GetRecordsBasedOnStatus(RecordStatus.Deleted);
			}
		}
		...
		/// </summary>
		public AgencyCollection ModifiedRecords
		{
			get
			{
				return (AgencyCollection)GetRecordsBasedOnStatus(RecordStatus.Modified);
			}
		}
		...
		/// </summary>
		public AgencyCollection NewRecords
		{
			get
			{
				return (AgencyCollection)GetRecordsBasedOnStatus(RecordStatus.New);
			}
		}
		...
		{
			dbCount = AgencyCollection.GetCountFromDB();
			return GetFromDB();
		}
		...
		public ClassGenExceptionCollection GetFromDB(string whereClause, ref long dbCount)
		{
			...
			dbCount = AgencyCollection.GetCountFromDB(whereClause);
			return GetFromDB();
		}
		...
		public ClassGenExceptionCollection GetFromDB() 
		{
			...
			try
			{
				sql = "SELECT " + (this.GetCollectionConfig.TopRecords > -1 ? "TOP " + this.GetCollectionConfig.TopRecords.ToString() + " " : "") +
					" [A].*, GETDATE() AS dateTimeObjectPopulated " + 
					"FROM tAgency [A] " + 
				...
				this.OnMasterPopulate("Agency", "Retrieving Full List Of Agencys", string.Empty);
				dt = DAL.SQLExecDataTable(cmd);
				...
				foreach (DataRow row in dt.Rows)
				{
					...
					this.OnDetailPopulate("Agency", 
						"Populating Agency: " + (++count).ToString("###,###,###,##0") + " of " + dt.Rows.Count.ToString("###,###,###,##0"), 
						string.Empty, 
						dt.Rows.Count, 
						count);
					...
					if (this.Contains(item.AgencyID)) { this.Remove(item.AgencyID); }
		...
		public ClassGenExceptionCollection AddUpdateAll(SqlConnection oConn, SqlTransaction oTrans)
		{
			...
			try
			{
				...
				{
					this.OnDetailDelete("Agency",
						"Deleting Agency: " + (this.Count - i + 1).ToString("###,###,###,##0") +
					...
					if (item.RecordStatus == RecordStatus.Deleted)
					{
						...
						else
						{
							...
							{
								err.RecordIndex = i + 1;
								err.RecordKey = item.AgencyID.ToString();
							}
				...
				// Update Method
				int count = 0;
				foreach (Agency item in this)
				{
					this.OnDetailUpdate("Agency",
						"Updating Agency: " + (count + 1).ToString("###,###,###,##0") +
						" of " + this.Count.ToString("###,###,###,##0"),
						string.Empty,
						this.Count,
						count + 1);
		...
		public new AgencyCollection Clone()
		{
			AgencyCollection newColl = new AgencyCollection();
		...
		/// </summary>
		/// <returns>A new AgencyCollection.</returns>
		public new AgencyCollection CopyWithNewIDs()
		{
			return (AgencyCollection)base.CopyWithNewIDs();
		}
		...
		/// <param name="field">The field type to get.</param>
		/// <returns>A list of values from the collection (sorted by value).</returns>
		public List<string> GetDistinctFromCollection(LEAD.AgencyField field)
		{
			...
			switch (field)
			{
				case LEAD.AgencyField.AgencyID:
					foreach (Agency item in this)
					{
					...
					break;
				case LEAD.AgencyField.AgencyKey:
					foreach (Agency item in this)
					{
						if (!String.IsNullOrEmpty(item.AgencyKey) && 
							!vals.ContainsKey(item.AgencyKey.Trim())) { vals.Add(item.AgencyKey.Trim(), item.AgencyKey.Trim()); }
					}
					break;
				case LEAD.AgencyField.AddedByUser:
					foreach (Agency item in this)
					{
					...
					break;
				case LEAD.AgencyField.UpdatedByUser:
					foreach (Agency item in this)
					{
		...
		/// <summary>
		/// Get all the local children (not from the DB) that are of type Contact
		/// </summary>
		/// <returns>A ContactCollection containing all the local children.</returns>
		public ContactCollection GetLocalChildren_Contact()
		{
			ContactCollection rtv = new ContactCollection();

			foreach (Agency parent in this)
			{
				foreach (Contact child in parent.Coll_Contact)
				{
					rtv.Add(child);
				}
			}
		...
		public ClassGenExceptionCollection BrokenRules
		{
			get 
			{
				...
				foreach (Agency item in this.ActiveRecords)
				{
					...
					if (exCollection.Count > 0)
					{
						foreach (ClassGenException ex in exCollection)
						{
							...
							ex.RecordKey = item.AgencyID.ToString();
						}
		...
		/// </summary>
		/// <returns>A AgencyCollection of current records.</returns>
		public AgencyCollection GetCurrent()
		{
		...
		public bool Exists(long agencyID)
		{
			return (this.Exists(new Predicate<Agency>(delegate (Agency obj) 
				{ return obj.AgencyID == agencyID; })));
		}
		...
		/// </summary>
		/// <param name="agencyID">A key field used to find the correct record.</param>
		public void Remove(long agencyID)
		{
			for (int i = this.Count - 1; i >= 0; i--)
			{
				if (this[i].AgencyID == agencyID)
				{
					this.RemoveAt(i);
				}
			}
		...
		{
			// Return the data table
			return EmptyDataTableWithIL<Agency>();
		}
		...
		/// <summary>
		/// Attempt to serialize the AgencyCollection object to the server
		/// </summary>
		/// <param name="obj">The AgencyCollection object to serialize</param>
		/// <returns>A string containing the return from the method</returns>
		public static string SerializeObject(AgencyCollection obj)
		{
			string rtv = string.Empty;
		...
		/// <summary>
		/// Attempt to Deserialize the AgencyCollection object from the server
		/// </summary>
		/// <param name="obj">The AgencyCollection object to deserialize</param>
		/// <param name="errors">The errors collection</param>
		/// <returns>A AgencyCollection object containing the return from the method</returns>
		public static AgencyCollection DeserializeObject(string obj, ref ClassGenExceptionCollection errors)
		{
			AgencyCollection rtv = null;

			try
			{
				rtv = Newtonsoft.Json.JsonConvert.DeserializeObject<AgencyCollection>(obj);
			}
			catch (Exception ex)
			{
				errors.Add(new ClassGenException("Error: " + ex.Message, ClassGenExceptionIconType.Critical));
		...
		public DataSet ToDataSet(bool includeDeleted)
		{
			...
			ContactCollection Coll_Contact = new ContactCollection();
			DataTable dtContact = Coll_Contact.ToEmptyDataTable();
			
			foreach (Agency item in this)
			{
				// Contact
				dt = item.Coll_Contact.ToDataTable(includeDeleted);
				foreach (DataRow row in dt.Rows)
				{
					newRow = dtContact.NewRow();
					foreach (DataColumn col in dtContact.Columns)
					{
						 newRow[col.ColumnName] = row[col.ColumnName];
					}
					dtContact.Rows.Add(newRow);
				}
			}
			
			// Add the datatables to the dataset
			ds.Tables.Add(dtContact);
			ds.Relations.Add("Agency_Contact", ds.Tables[0].Columns["AgencyID"], ds.Tables[ds.Tables.Count - 1].Columns["AgencyID"]);
		...
		private void _backgroundWorker_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			sql = 
				"IF object_id('tempdb..#Agency') IS NOT NULL  DROP TABLE #Agency; " + 
					"CREATE TABLE #Agency (iAgencyID bigint, iItemIndex INT IDENTITY(1,1) PRIMARY KEY CLUSTERED);  " + 
					"INSERT INTO #Agency (iAgencyID) SELECT iAgencyID FROM tAgency" + 
					(!String.IsNullOrEmpty(this.GetCollectionConfig.WhereClause) ? " WHERE " + this.GetCollectionConfig.WhereClause : "") + ";  " + 
					"SELECT COUNT(*) AS iCount FROM #Agency "; 
			cmd = new SqlCommand(sql);
		...
		private void _backgroundWorker_ProgressChanged(object sender, ProgressChangedEventArgs e)
		{
			if (e.UserState != null)
			{
				AgencyCollection_GetAsyncObject obj = (AgencyCollection_GetAsyncObject)e.UserState;
				this.OnGetThreadedProgress(obj.CurrentIndex, obj.TotalCount, obj.AgencyCollection);
			}
		}
			...
			int pageSize)
		{
			...
			AgencyCollection coll = new AgencyCollection();
			Agency item = new Agency();
			...
			while (currentIndex < totalCount)
			{
				...
				sql = "SELECT [A].*, GETDATE() AS dateTimeObjectPopulated " + 
					"FROM tAgency [A] " + 
					"INNER JOIN #Agency TEMP ON [A].[iAgencyID] = [TEMP].[iAgencyID] " +
				...
				// Populate the local Agency collection
				coll = new AgencyCollection();
				foreach (DataRow row in dt.Rows)
				{
					item = new Agency();
					item.Get(row);
				...
				// Report the progress
				AgencyCollection_GetAsyncObject obj = new AgencyCollection_GetAsyncObject(currentIndex,
					totalCount, 
					coll);
		...
		/// </summary>
		public delegate void GetThreadedProgressEventHandler(object sender, Agency_GetCollection_EventArgs e);
		/// <summary>
		...
		/// </summary>
		public void OnGetThreadedProgress(int currentIndex, int totalCount, AgencyCollection coll)
		{
			if (GetThreadedProgress != null)
			{
				// Add the items found in the collection to the current set
				foreach (Agency a in coll)
				{
					this.Add(a);		// Add the Agency
				}

				Agency_GetCollection_EventArgs e = new Agency_GetCollection_EventArgs(currentIndex, totalCount, coll);
		...
		public string GetAsXML(bool includeBaseProperties)
		{
			...
			sb.Insert(0, "<AgencyCollection>" + Environment.NewLine);
			sb.Append("</AgencyCollection>" + Environment.NewLine);
			...
			foreach (Agency item in this)
			{
				switch (item.RecordStatus)
				{
					case RecordStatus.Current:
						...
						sbRead.Append("<Agency>" + Environment.NewLine);
						sbRead.Append("\t<AgencyID>" + item.AgencyID.ToString() + "</AgencyID>" + Environment.NewLine);
						sbRead.Append("\t<AgencyKey>" + item.AgencyKey + "</AgencyKey>" + Environment.NewLine);
						...
						sbRead.Append("\t<AddedByUser>" + (item.AddedByUser != null ? item.AddedByUser : "NULL") + "</AddedByUser>" + Environment.NewLine);
						sbRead.Append("</Agency>" + Environment.NewLine);
					...
					case RecordStatus.Deleted:
						sbDeleted.Append("<Agency>" + Environment.NewLine);
						sbDeleted.Append("\t<AgencyID>" + item.AgencyID.ToString() + "</AgencyID>" + Environment.NewLine);
						sbDeleted.Append("\t<AgencyKey>" + item.AgencyKey + "</AgencyKey>" + Environment.NewLine);
						...
						sbDeleted.Append("\t<AddedByUser>" + (item.AddedByUser != null ? item.AddedByUser : "NULL") + "</AddedByUser>" + Environment.NewLine);
						sbDeleted.Append("</Agency>" + Environment.NewLine);
					...
					case RecordStatus.New:
						sbNew.Append("<Agency>" + Environment.NewLine);
						sbNew.Append("\t<AgencyID>" + item.AgencyID.ToString() + "</AgencyID>" + Environment.NewLine);
						sbNew.Append("\t<AgencyKey>" + item.AgencyKey + "</AgencyKey>" + Environment.NewLine);
						...
						sbNew.Append("\t<AddedByUser>" + (item.AddedByUser != null ? item.AddedByUser : "NULL") + "</AddedByUser>" + Environment.NewLine);
						sbNew.Append("</Agency>" + Environment.NewLine);
		...
		/// <returns>The completed AgencyCollection object.</returns>
		public static AgencyCollection LoadFromXML(string xml)
		{
			AgencyCollection coll = new AgencyCollection();
			System.Xml.XmlDocument doc = new System.Xml.XmlDocument();
			doc.LoadXml(xml);
			...
			foreach (System.Xml.XmlNode nodeMain in doc.ChildNodes)
			{
				...
				foreach (System.Xml.XmlNode node in nodeMain.ChildNodes)
				{
					...
					Agency item = Agency.LoadFromXML(node.OuterXml);
		...
		protected virtual void Dispose(bool disposing)
		{
			...
			if (!this._disposed)
			{
				...
				if (disposing)
				{
					...
					KeyedCollection = new Dictionary<long, Agency>();
	...
	#endregion Agency Collection

	#region Event Handler for Async Object
	/// <summary>
	/// Custom class for Agency event args such as population
	/// </summary>
	[Serializable]
	public class Agency_GetCollection_EventArgs : EventArgs
	{
		private int _totalCount = 0;
		private int _currentIndex = 0;
		private AgencyCollection _coll = new AgencyCollection();

		/// <summary>
		/// Creates a new Agency_GetCollection_EventArgs with the specified properties.
		/// </summary>
		...
		public Agency_GetCollection_EventArgs(int currentIndex, int totalCount, AgencyCollection coll)
		{
		...
		/// <summary>
		/// Creates a new empty Agency_GetCollection_EventArgs.
		/// </summary>
		public Agency_GetCollection_EventArgs()
		{
		}
		...
		/// </summary>
		public AgencyCollection AgencyCollection
		{
			get { return _coll; }
		}
	}
	...
	/// <summary>
	/// An object to populate a AgencyCollection asynchronously.
	/// </summary>
	public class AgencyCollection_GetAsyncObject
	{
		private int _currentIndex = 0;
		private int _totalCount = 0;
		private AgencyCollection _coll = new AgencyCollection();
		...
		/// <summary>
		/// Creates a new Agency_GetAsyncObject with the specified properties.
		/// </summary>
		/// <param name="currentIndex">The index of the current record to be populated.</param>
		/// <param name="totalCount">The total number of records to be collected.</param>
		/// <param name="coll">The collection holding the records.</param>
		public AgencyCollection_GetAsyncObject(int currentIndex, int totalCount, AgencyCollection coll)
		{
			_currentIndex = currentIndex;
			_totalCount = totalCount;
			_coll = coll;
